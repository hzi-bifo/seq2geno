...
def parse_gff(gff_f, tmp_gff_f):
    ...
    good_gff_df= pd.DataFrame([l.strip().split('\t') for l in good_lines if
                               re.match('#', l) is None])
    good_gff_df.columns=['CHROM', 'source', 'feature', 'Start', 'Stop', 'score',
    ...
    good_gff_df['secondary_key']= good_gff_df.apply(lambda r: '\t'.join(r), axis= 1)
    # for field_map_wrapper.edit:line53 
    good_gff_df.sort_values(by=['Start', 'secondary_key'], ascending=True, inplace= True)
    # for field_map.edit.py:line31
    good_gff_df.drop_duplicates(subset= ['Start'], inplace= True, keep= 'first')
...
def parse_tab(tab_f):
    ...
    with open(tab_f, 'r') as tab_fh:
        cmt_lines= [l for l in tab_fh.readlines() if re.match('@', l)]
