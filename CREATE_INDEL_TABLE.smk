...
#        genes_list='core_genes_50.txt',
        ...
        #gene_dna_seqs= expand('{TMP_D}/{assembler}/prokka/{strain}/{strain}.ffn',
            #TMP_D=TMP_D, strain= STRAINS, assembler= SOFTWARE['assembler'])
        #TMP_D=TMP_D, strain= STRAINS, assembler= config['assembler'])
        roary_gpa= lambda wildcards: os.path.join(wildcards.TMP_D,
            SOFTWARE['gene_sorter'],'gene_presence_absence.csv'),
        gene_dna_seqs= lambda wildcards: [os.path.join(wildcards.TMP_D, strain,
