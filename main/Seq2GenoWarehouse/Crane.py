...
import sys
sys.path.append(r'/home/thkuo/test_seq2geno/seq2geno/')
sys.path.append(r'/home/thkuo/test_seq2geno/seq2geno/main')

from main.UserOptions import main as AskArg
from main.UserOptions import arguments,make_parser,parse_arg_yaml,check_primary_args
def test_material(f):
    try:
...
# fix the paths because the server will need to go editting them immediately
...
redirected_rna_reads_dir= os.path.join(
    new_dir, 'reads', 'rna')
# The paths below should be the 'configured' values; that is, after the '__' are
# replaced with the exact path
...
new_config_yaml_dict['general']['wd']= '__/{}'.format(os.path.basename(args.wd))
    ...
    for l in tqdm(rna_list_lines):
        ...
        try:
            ...
            new_rna_reads[d[0]]= '__/{}'.format(
                os.path.join(os.path.relpath(redirected_rna_reads_dir, new_dir),
